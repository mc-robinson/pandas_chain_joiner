...
Usage: python pandas_chain_joiner.py pdb_filename.pdb fasta_filename.fasta [options]
...
def main(argv):
	...
	for opt, arg in opts:
		...
		if opt in ('-f','--fixed_automodel'):
			...
			subprocess.call(['python', 'pandas_make_alignment.py', str(sys.argv[1]), pdb_id + ".seq", str(sys.argv[2])])

			#make the model
			subprocess.call(['python', 'pandas_make_model.py', str(sys.argv[1]),'-f'])
			...
			#make the model
			subprocess.call(['python', 'pandas_make_model.py', str(sys.argv[1]),'-l'])
			...
			#make the model
			subprocess.call(['python', 'pandas_make_model.py', str(sys.argv[1]),'-a'])
